          ...
          for (const [line, patch] of patches) {
            ...
            try {
              ...
              try {
                ...
                core.warning("Failed to parse structured JSON from OpenAI");
              }
              ...
              if (reviewData) {
                ...
                for (const category of categories) {
                  const issues = reviewData[category];
                  if (issues && issues.length > 0) {
                    ...
                    for (const issue of issues) {
                      ...
                      formatted += `- ${icon} **${issue.severity}**: ${issue.description}\n`;
                    }
                    formatted += "\n";
                  }
                }

                formatted += `\nConfidence Score: ${reviewData.confidence_score}`;
